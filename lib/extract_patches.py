...
def load_file_path_txt(file_path):
    ...
    with open(file_path, 'r') as file_to_read:
        while True:
            ...
            if not lines:
                break
            img,gt,fov = lines.split(' ')       #拆分成3个地址
            img_list.append(img)
            gt_list.append(gt)
            fov_list.append(fov)
...
def load_data(data_path_list_file):
    ...
    img_list, gt_list, fov_list = load_file_path_txt(data_path_list_file)   #获取原图、标签、遮罩地址
    imgs = None
    groundTruth = None
    FOVs = None
    for i in range(len(img_list)):
        ...
        if len(gt.shape)==3:   #gt需要单通道图片
            gt = gt[:,:,0]     #生成通道图，降成2维
        fov = np.asarray(readImg(fov_list[i]))   #读取图片
        if len(fov.shape)==3:
            fov = fov[:,:,0]
        ...
        FOVs = np.expand_dims(fov,0) if FOVs is None else np.concatenate((FOVs,np.expand_dims(fov,0)))  #把图片收集起来
    ...
    groundTruth = np.expand_dims(groundTruth,1)  #生成4维
...
def get_data_train(data_path_list,patch_height,patch_width,N_patches,inside_FOV):
    train_imgs_original, train_masks, train_FOVs = load_data(data_path_list)  #获取原图、标签、遮罩
    # save_img(group_images(train_imgs_original[0:20,:,:,:],5),'imgs_train.png')#.show()  #check original train imgs 

    train_imgs = my_PreProc(train_imgs_original)
    train_masks = train_masks/255.
    train_FOVs = train_FOVs//255  #变为0-1
...
def extract_random(full_imgs,full_masks,full_FOVs, patch_h,patch_w, N_patches, inside='not'):
    ...
    for i in range(full_imgs.shape[0]):  #loop over the all images 随机裁剪出200000张图
        k=0
        while k <patch_per_img:
            x_center = random.randint(0+int(patch_w/2),img_w-int(patch_w/2))
            y_center = random.randint(0+int(patch_h/2),img_h-int(patch_h/2))
            #check whether the patch is contained in the FOV
            if inside=='center' or inside == 'all':  #判断中心点时候在fov里
                if not is_patch_inside_FOV(x_center,y_center,full_FOVs[i,0],patch_h,patch_w,mode=inside):
                    continue
            patch = full_imgs[i,:,y_center-int(patch_h/2):y_center+int(patch_h/2),x_center-int(patch_w/2):x_center+int(patch_w/2)]  #根据中心点进行随机裁剪64*64的分块
...
def get_data_test_overlap(test_data_path_list, patch_height, patch_width, stride_height, stride_width):
    test_imgs_original, test_masks, test_FOVs= load_data(test_data_path_list) #获取测试集的原图、标签、遮罩

    test_imgs = my_PreProc(test_imgs_original)
    test_masks = test_masks/255.
    test_FOVs = test_FOVs//255
    #extend both images and masks so they can be divided exactly by the patches dimensions
    test_imgs = paint_border_overlap(test_imgs, patch_height, patch_width, stride_height, stride_width) #对测试集图片进行填补，以便于有序分块
...
# extend both images and masks so they can be divided exactly by the patches dimensions
def paint_border_overlap(full_imgs, patch_h, patch_w, stride_h, stride_w):     #对图像进行填补，以便于有序分块
    assert (len(full_imgs.shape)==4)  #4D arrays
    assert (full_imgs.shape[1]==1 or full_imgs.shape[1]==3)  # check the channel is 1 or 3
    img_h = full_imgs.shape[2]  #height of the image
    img_w = full_imgs.shape[3] #width of the image
    leftover_h = (img_h-patch_h)%stride_h  #leftover on the h dim 取余数
...
def extract_ordered_overlap(full_imgs, patch_h, patch_w,stride_h,stride_w):  #对测试图片进行分块，分成19840块图像
    assert (len(full_imgs.shape)==4)  #4D arrays
    assert (full_imgs.shape[1]==1 or full_imgs.shape[1]==3)  #check the channel is 1 or 3
