...
class Evaluate():
    def __init__(self,save_path=None):
        self.target = None
        self.output = None
        self.dataset=parse_args().test_data_path_list
    ...
    #对测试数据进行抽样，降低评估指标计算难度
    def sampling(self,target,output):
        # 检查target和output是numpy数组
        assert isinstance(target, np.ndarray)
        assert isinstance(output, np.ndarray)
        # 检查target和output的形状是否相同
        assert target.shape == output.shape
        # 数据集的10%
        sample_size = int(target.size * 0.1)
        # 生成随机索引
        indices = np.random.choice(target.size, size=sample_size, replace=False)

        # 使用随机索引抽取样本
        target_sample = target[indices]
        output_sample = output[indices]
        return target_sample,output_sample
    ...
    def add_batch(self,batch_tar,batch_out):
        ...
        if 'HRF' in self.dataset:
            self.target, self.output = self.sampling(self.target, self.output)  # 对测试数据进行抽样，降低评估指标计算难度
            self.target = (self.target > 0.5).astype(np.int)  # 将target转换为0和1
    ...
    def auc_roc(self,plot=True):
        ...
        if plot and self.save_path is not None:
            ...
            np.savetxt("FPR.txt", fpr, fmt='%f',
                       delimiter=',')
            np.savetxt("TPR.txt", tpr, fmt='%f',
                       delimiter=',')
    ...
    def mcc(self):
        ...
        pred = self.output >= self.threshold_confusion
        # 计算MCC
        mcc_value = matthews_corrcoef(self.target, pred)
        return mcc_value
    ...
    def save_all_result(self,plot_curve=True,save_name=None):
        ...
        return OrderedDict([("AUC_ROC",AUC_ROC),("AUC_PR",AUC_pr),
                            ...
                            ("precision",precision), ("MCC", mcc_value)
                            ])